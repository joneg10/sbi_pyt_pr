...
def get_ligands_from_structure(structure):
...
def get_ligand_binding_site_atoms(structure):
    '''Returns list of atoms in the ligand binding site'''
    
    list_of_ligands = get_ligands_from_structure(structure) # get ligands from the structure
...
def get_structure_environments(structure):
    '''Returns dictionary of environments of each residue in the structure'''
    
    ligand_names = get_ligands_from_structure(structure)
    ligand_binding_atoms = get_ligand_binding_site_atoms(structure)
        ...
        for residue in structure.get_residues():
            ...
            if residue.get_resname() not in ligand_names and residue.get_resname() != 'HOH':  # Exclude water residues and ligand residues
                for selected_atom in residue.get_atoms():
                    nearby_atoms = ns.search(selected_atom.coord, 4)  # Change the radius if needed
