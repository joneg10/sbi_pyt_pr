...
class StructureAnalysis:
        ...
        for residue in self.structure.get_residues():
            ...
            if len(list_of_ligands) > 0:    
                if residue_name in list_of_ligands:
                    ...
                    nearby_protein_atoms = list(set(nearby_protein_atoms))
    ...
    def get_structure_environments(self):
        ...
        if len(ligand_names) > 0:
            ...
            for residue in self.structure.get_residues():
                for atom in residue.get_atoms():
                    if atom.get_serial_number() not in serial_numbers_lbs and residue.get_resname() in self.standard_residues:
                        non_lbs_atoms.append(atom.get_serial_number())

            # sample the same number of atoms from non_lbs_atoms as from lbs_atoms
            ...
            for residue in self.structure.get_residues():
                ...
                if residue.get_resname() not in ligand_names and residue.get_resname() != 'HOH':  # Exclude water residues and ligand residues
                    ...
                    for selected_atom in residue.get_atoms():
                        ...
                        if selected_atom.get_serial_number() in non_lbs_atoms or selected_atom.get_serial_number() in serial_numbers_lbs:
                            mycounter += 1
                            nearby_atoms = self.ns.search(selected_atom.coord, radio)  
                            ...
                            # use a chimera recognizable code as key
